...
def grounded_segmentation(
    ...
    box = get_boxes(detections)[0][0]
    detection = detections[0]
    # Create masks equal to bounding boxes
    ...
    detection.mask = mask
    # detections = segment(
    #     segmentator, segment_processor, image, detections, polygon_refinement
    # )

    return np.array(image), [detection]
...
class CustomFluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if self.attn_enforce != 1.0:
            attn_probs = (
                torch.einsum("bhqd,bhkd->bhqk", query, key) * attn.scale
            ).softmax(dim=-1)
            img_attn_probs = attn_probs[:, :, -self.num_pixels :, -self.num_pixels :]
            img_attn_probs = img_attn_probs.reshape(
                (
                    batch_size,
                    attn.heads,
                    self.height,
            ...
            img_attn_probs[:, :, :, self.width // 2 :, :, : self.width // 2] *= (
                self.attn_enforce
            )
            img_attn_probs = img_attn_probs.reshape(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_image_path", type=str, default="./assets/bear_plushie.jpg"
    )
    parser.add_argument("--subject_name", type=str, default="bear plushie")
    parser.add_argument(
        "--target_prompt",
        type=str,
        default="a photo of a bear plushie surfing on the beach",
    )
    ...
    def segment_image(image, object_name):
        ...
        segment_result = (
            image_array * np.expand_dims(detections[0].mask / 255, axis=-1)
            + np.ones_like(image_array)
            * (1 - np.expand_dims(detections[0].mask / 255, axis=-1))
            * 255
        )
    ...
    reference_image = (
        load_image(args.input_image_path).resize((width, height)).convert("RGB")
    )

    ctrl_scale = args.ctrl_scale
    segmented_image = segment_image(reference_image, subject_name)
    mask_image = np.concatenate(
        [np.zeros((height, width, 3)), np.ones((height, width, 3)) * 255], axis=1
    )
